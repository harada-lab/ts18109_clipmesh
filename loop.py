...
def loop(cfg):
    ...
    model, _ , preprogress = open_clip.create_model_and_transforms('ViT-B-32-quickgelu', pretrained='laion400m_e32',device=device)
    ...
    for it in t_loop:
        ...
        with torch.no_grad():
            cams_norm = params['campos'] / np.linalg.norm(params['campos'],ord=2).to(device) #cams_norm の中身は[x座標,y座標,z座標]をそれぞれの最大値を1にしたもの。
